...
void App_AgarioGame::Start()
{
	...
	//m_pSmartAgent->SetSteeringBehavior
	//m_pSmartAgent->MarkForUpgrade(5);
	...
	FoodNearBy* pFoodNearByCondition = new FoodNearBy;
	FoodGone* pFoodGoneCondition = new FoodGone;
	BiggerAgentNearby* pBiggerAgentNearbyCondition = new BiggerAgentNearby;
	BiggerAgentGone* pBiggerAgentGoneCondition = new BiggerAgentGone;
	...
	pStateMachine->AddTransition(pSeekFoodState, pWanderState, pFoodGoneCondition);

	// Flee if a bigger agent is nearby
	pStateMachine->AddTransition(pSeekFoodState, pFleeState, pBiggerAgentNearbyCondition);
	pStateMachine->AddTransition(pWanderState, pFleeState, pBiggerAgentNearbyCondition);

	// Return to wandering if there's no bigger agents nearby anymore
	pStateMachine->AddTransition(pFleeState, pWanderState, pBiggerAgentNearbyCondition);
...
Blackboard* App_AgarioGame::CreateBlackboard(AgarioAgent* a)
{
	...
	pBlackboard->AddData("AgentPtr", a);
	pBlackboard->AddData("FoodVecPtr", &m_pFoodVec);
	pBlackboard->AddData("FoodPtr", static_cast<AgarioFood*>(nullptr));
	pBlackboard->AddData("AgentVecPtr", &m_pAgentVec);  // List of all agents, to check for bigger agents
	pBlackboard->AddData("FleeAgentPtr", static_cast<AgarioAgent*>(nullptr));  // Pointer to the agent that is bigger than the current agent and we evade
